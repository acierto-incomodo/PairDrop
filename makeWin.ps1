...
if ($exeFile) {
    Copy-Item -Path $exeFile.FullName -Destination (Join-Path $exeFile.DirectoryName "PairDrop-Setup.exe") -Force
    Write-Host "Se ha creado una copia: PairDrop-Setup.exe"
}